...
impl Op {
    pub fn to_string(&self) -> String {
        match self {
            ...
            &Op::FloatSub{lhs, rhs, target} => format!("{{{2}}} <= {{{0}}} - {{{1}}} as Float", lhs, rhs, target),
            &Op::FloatMul{lhs, rhs, target} => format!("{{{2}}} <= {{{0}}} + {{{1}}} as Float", lhs, rhs, target),
            &Op::FloatDiv{lhs, rhs, target} => format!("{{{2}}} <= {{{0}}} + {{{1}}} as Float", lhs, rhs, target),
            &Op::FloatLoad{register, constant} => format!("{{{0}}} <= {{{1}}} as Float", register, constant),
            &Op::FloatLessEqual{lhs, rhs, target} => format!("{{{2}}} <= {{{0}}} <= {{{1}}} as Float", lhs, rhs, target),
            &Op::FloatLessThan{lhs, rhs, target} => format!("{{{2}}} <= {{{0}}} < {{{1}}} as Float", lhs, rhs, target),
            &Op::Goto{position} => format!("goto {{{0}}}", position),
            &Op::IntAdd{lhs, rhs, target} => format!("{{{2}}} <= {{{0}}} + {{{1}}} as Int", lhs, rhs, target),
            &Op::IntCmp{lhs, rhs, target} => format!("{{{2}}} <= {{{0}}} == {{{1}}} as Int", lhs, rhs, target),
            &Op::IntSub{lhs, rhs, target} => format!("{{{2}}} <= {{{0}}} - {{{1}}} as Int", lhs, rhs, target),
            &Op::IntMul{lhs, rhs, target} => format!("{{{2}}} <= {{{0}}} + {{{1}}} as Int", lhs, rhs, target),
            &Op::IntDiv{lhs, rhs, target} => format!("{{{2}}} <= {{{0}}} + {{{1}}} as Int", lhs, rhs, target),
            &Op::IntLoad{register, constant} => format!("{{{0}}} <= {{{1}}} as Int", register, constant),
            ...
            &Op::ModuleLoadValue{ref module_name, ref name, target} => format!("{{{2}}} <= {0}.{1}", module_name, name, target),
            &Op::Noop{} => format!("noop"),
