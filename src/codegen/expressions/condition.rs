...
pub fn gen_condition(vm: &mut VM, c: &Condition, scope: &mut Scope, ops: &mut Vec<Op>) -> LocalObject {
    let cond_object = evaluate_expr(vm, &(c.expression), scope, ops);
    let cond_index = ops.len();
    // push a noop, replace it later with the correct branch.
    ops.push(Op::Noop{});
    gen_statement_list(vm, &(c.true_block), scope, ops);
    let end_true_block_index = ops.len();
    ops.push(Op::Noop{});
    gen_statement_list(vm, &(c.false_block), scope, ops);
