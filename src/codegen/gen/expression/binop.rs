...
pub fn generate_binop(c: &mut Context, binop: &BinOp) -> CGResult<LocalObject> {
    ...
    return if left.typ == types::FLOAT_TYPE.clone() {
        let object = c.block.scope.allocate_local(types::FLOAT_TYPE.clone());
        match binop.op {
            ...
            BinaryOperator::Neq => {
                let cmp = c.block.scope.allocate_local(types::BOOL_TYPE.clone());
                c.block.ops.push(Op::FloatCmp{lhs: left.index, rhs: right.index, target: cmp.index});
                let second_cmp = c.block.scope.allocate_local(types::BOOL_TYPE.clone());
        ...
        let object = c.block.scope.allocate_local(types::INT_TYPE.clone());
        match binop.op {
